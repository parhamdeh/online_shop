...
from online_shop.users.selectors.user_selectors import get_user_cart, get_user_order, get_user_profile, get_user_list, get_user_wallet
from online_shop.users.apis.id.users_id_serializer import CartOutput, OrderOutputSerializer, ProfileSerializer, UserOutput, WalletOutputSerializer
...
@extend_schema(
    ...
    description="Retrieve the shopping cart of a specific user.",
    responses={
        200: CartOutput,
        403: OpenApiResponse(description="Permission denied"),
...
class RetrieveShopCartAPIView(RetrieveAPIView):
    permission_classes = [IsALLowToSeeProfile]
    throttle_classes = [UserRequestThrottle]
    serializer_class = CartOutput
    renderer_classes = [CustomResponseRenderer]

    def get_object(self):
        try:
            cart = get_user_cart(user_id=self.kwargs["user_id"]).get()
        except Exception as ex:
            logger.exception(f"database error {ex}")
            raise ApplicationError(f"database error {ex}")
